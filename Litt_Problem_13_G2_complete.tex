...
Until the beginning of \cref{sec:pair-splitting}, fix
$b=(A,B,c)\in\cB(\CC)$ and suppress it from the notation.  Thus
\[
...
\[
 X_0X_2-X_1^2=0,
\]
which has rank three and one ruling.  Every degree-three line bundle $A$
with $h^0(C,A)\ge2$ is base-point-free: otherwise, for a base point $p$,
the degree-two line bundle $A(-p)$ would have at least two sections and
would define a $g^1_2$, contrary to nonhyperellipticity.  Geometric
Riemann--Roch therefore identifies every $g^1_3$ with a ruling of the
canonical quadric.  Its unique ruling is the one cut out by $|\kappa|$, so
$\kappa$ is the unique $g^1_3$ on $C$.
...
reduced.  The divisor sequence, together with Kodaira vanishing and Serre
duality for $L^{-1}$, gives $H^0(X,\mathcal O_X)=\CC$; thus $X$ is
connected.  Suppose that $X$ were
...
The uniqueness of the $g^1_3$ shows that every $M\ne0$ in $X$ satisfies
$h^0(C,\kappa\otimes M)=1$, whereas the Abel fibre over $0$ is precisely
$\Gamma=|\kappa|$.  Base change of the Abel map therefore gives an
isomorphism
\[
 N\setminus\Gamma\xrightarrow{\sim}X\setminus\{0\}.
\]
The surface $N$ is reduced: it is smooth away from $D_0$, and its germ at
$D_0$ is the integral cubic cone.  It is also a Cartier divisor in the
smooth variety $C^{(3)}$, hence is pure of dimension two; no irreducible
component can be contained in the curve $\Gamma$.  The displayed integral
dense open then shows that $N$ is integral.  Consequently $\rho$ is a
proper birational resolution whose only exceptional curve is $E_{\exc}$.
Since this curve is elliptic with self-intersection $-2$, the resolution is
minimal and the simple-elliptic classification identifies the germ with
type $\wt E_7$ \cite{Laufer1977,Pinkham1977}.
\end{proof}
...
\end{lemma}
\begin{proof}
The local Euler obstruction is invariant under analytic isomorphism, so it
may be computed in the standard hypersurface embedding of a $\wt E_7$
simple-elliptic germ,
\[
 z^2+f_4(x,y)=0,
\]
where $f_4$ is a square-free binary quartic
\cite{Laufer1977,Pinkham1977}.  In these coordinates take a general linear
form $\ell=z-ax-by$.  On $\ell=0$ the quadratic term $(ax+by)^2$ has rank
one, and square-freeness makes the restriction of $f_4$ to its null line
nonzero for general $(a:b)$.  The analytic splitting lemma therefore
reduces the plane-curve germ to $s^2+t^4=0$, namely type $A_3$, whose Milnor
number is $3$.
...
The exceptional elliptic curve contributes to $H^1(S)$.  Its contribution is
determined before introducing the generic twist.
...
of $s(\pm a)$, depending on the action convention; both values vanish by
\eqref{eq:symmetry} and \eqref{eq:kernel-on-theta}.  The orbit of the
...
From this point, $\IC_X^H$ also denotes its scalar extension to a polarizable
complex Hodge module.  A unitary rank-one local system underlies a unique
polarizable complex variation of Hodge structure of weight zero and type
$(0,0)$.  Thus twisting by $\mathcal L_\alpha$ is defined for polarizable
complex Hodge modules, and its cohomology has a functorial complex Hodge
decomposition \cite[Sections~2.1 and~2.5]{KLM2026}.
...
\]
For later use in the norm calculation, the basis argument is included.  On $D$ one has $x=Y=w^3$ and
...
\[
 a_j=u-v\omega^j,
\]
and neither $a_1$ nor $a_2$ vanishes at the generic point of the
irreducible degree-eight curve $\mathscr R_{\CC}$.  Shrink once so that
$a_1a_2\ne0$.  Then
\[
 b_j=-(u-v\omega^j)q^3
     +v(1-\omega^j)
...
affine roots in \eqref{eq:restriction-two-conjugates} are therefore distinct.
...
\end{lemma}
\begin{proof}
For a fixed section, its effective zero divisor on the smooth curve $C$ has
only finitely many effective degree-three subdivisors, so $\Psi$ has finite
fibres.  Recall $h$ and the identities in
\eqref{eq:structural-pencil}.
...
\]
 ...
 Reducedness, transversality, and disjointness fail on proper closed subsets
 of this projective line, so the exceptional parameters of the structural
 pencil form a finite set.  If
...
component maps to the residual octic $V(F_8)$.  Let $Z$ be such a component.
Its image is one-dimensional because $\Psi|_Z$ has finite fibres, and hence
is dense because $\mathscr R$ is geometrically integral.  Thus
$Z\cap\Psi^{-1}(\mathscr G)$ is a dense open subset of $Z$.  Over
$\mathscr G$, \cref{prop:abel-incidence,cor:two-admissible-branches}
identify the incidence scheme with the disjoint union of two irreducible
branches.  The irreducibility of $Z$ places its dense open in exactly one of
them, and dimensional equality then makes it dense in that branch.  Hence
$Z$ is the closure of one of the two admissible branches.  Conversely, each
branch closure is an irreducible component.  These are therefore exactly the
two nonexceptional components.
...
coincide, so their Gauss covectors would be equal.  The equivariance identity
\[
 \gamma_X(\tau x)=\tau\cdot\gamma_X(x)
\]
then makes that common covector projectively fixed by $\tau$.  The general
...
point.  A dense constructible subset of an irreducible noetherian scheme
contains the generic point, so this image is not dense and its closure is
proper.  Remove this closure.  Then
$\mathcal J=0$, and
\[
...
The shifts are fixed by the identity
\[
 \widetilde K*\widetilde K
 \simeq R\widetilde m_*\QQ_{S\times S}[4].
\]
Proper base change therefore identifies
\[
...
canonically $K$, and the commutativity constraint exchanges them.  Their
anti-invariant part is therefore canonically one copy of $K$.  Since $t$ is
a smooth point of the surface $X$, its degree-$-2$ stalk is the line
generated by $[C_{1,t}]-[C_{2,t}]$.  Taking dimensions in this direct-sum
decomposition gives
\begin{equation}
 \dim\cH^{-2}(K*K)_{-,t}=1.
...
semisimplicity \cite[Th\'eor\`eme~6.2.5]{BBD1982} and
\cite[Th\'eor\`eme~5.3.1]{Saito1988}, it is a direct sum of shifted simple
...
\eqref{eq:principal-alt-rank}.  Only finitely many simple summands occur.
Choose $a\in U^+$ in the two-dimensional stratum of a common Whitney
stratification subordinate to $X$ and to all these summands.  Normal
nonsingularity along this stratum identifies a sufficiently small
stratified analytic neighbourhood with
\[
 (B^2\times\Delta,B^2\times\{0\}),
\]
where $B^2\subset X_{\reg}$ is a ball and $\Delta$ is a normal disk.
Intermediate extension commutes with this external product.  Hence a
full-support simple perverse summand is locally
\[
...
\eqref{eq:principal-alt-rank} has rank one.  All preceding shrinkings may be
made Zariski open.  Thus $U^+\cap X_{\mathcal V}$ is a nonempty Zariski-open
subset of the smooth irreducible variety $X_{\mathcal V}$ and is connected.
Its restriction there is the constant local system found in
\cref{lem:split-fibre-top}.  The complement in $X_{\mathcal V}$ has complex
codimension at least one.  By general position, every loop in
$X_{\mathcal V}$ is homotopic to one avoiding this complement, so inclusion
induces a surjection
\[
 \pi_1(U^+\cap X_{\mathcal V})\twoheadrightarrow
 \pi_1(X_{\mathcal V}).
\]
...
The simple perverse sheaf $K$ is nondivisible in the sense of
\cite[Section~3.3]{JKLM2025}: if $t_a^*K\simeq K$, then $X+a=X$, and
\eqref{eq:stabilizer} gives $a=0$.  By
...
Put $\mu(z)=h(z,z^{-1})$.  The antidiagonal cocharacter must lie in the
common derived group rather than merely in the Hodge group.  This follows from
the determinant.  Since $\GGm$ is connected,
...
\subsection{An admissible specialization}
\label{app:admissible-specialization}
...
\]
so it is reduced.  On the normalized chart,
\[
 (U,Z)=(u/v,z/v)=(3/5,1/5)=(11,8)
 \quad\text{in }\mathbf F_{13}^2.
\]
Consequently
\[
 \bigl(q_{ij}^{\mathrm{hom}}(3,5,1)\bigr)_{ij}
 =\left.5^3\bigl(q_{ij}(U,1,Z)\bigr)_{ij}
   \right|_{(U,Z)=(11,8)}
 =(5,1,7,7,0,2),
\]
where the entries are ordered as
$q_{00},q_{01},q_{02},q_{11},q_{12},q_{22}$.  The residual conic therefore
factors as
\[
 5+x+7Y+7x^2+2Y^2
 =5(1+9x+Y)(1+12x+3Y).
...
\bibitem{Litt13}
Daniel Litt,
\emph{Problem no.~13}, Problems I Like, \url{https://www.problemsilike.com/13}, accessed August 11, 2026.

\bibitem{Macdonald1962}
I.~G. Macdonald,
\emph{Symmetric products of an algebraic curve}, Topology \textbf{1} (1962), 319--343,
\newblock \url{https://doi.org/10.1016/0040-9383(62)90019-8}.

\bibitem{Pinkham1977}
Henry C. Pinkham,
\emph{Simple elliptic singularities, del Pezzo surfaces and Cremona transformations}, in \emph{Several complex variables}, Proc. Sympos. Pure Math., vol.~30, Part~1, American Mathematical Society, Providence, RI, 1977, pp.~69--71.

\bibitem{Saito1988}
Morihiko Saito,
\emph{Modules de Hodge polarisables}, Publ. Res. Inst. Math. Sci. \textbf{24} (1988), no.~6, 849--995,
\newblock \url{https://doi.org/10.2977/PRIMS/1195173930}.

\bibitem{stacks-project}
The Stacks Project Authors,
\emph{Stacks Project}, \url{https://stacks.math.columbia.edu}, accessed August 11, 2026.
