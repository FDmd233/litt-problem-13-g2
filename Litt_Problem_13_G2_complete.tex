...
\begin{abstract}
We construct a family of complex abelian threefolds from cyclic triple covers
\[
  C:\quad w^3=y-c,\qquad c\ne0,
\]
of elliptic curves $y^2=x^3+Ax+B$.  The induced Prym polarization has type
$(1,1,3)$.  Its centred theta section $X$ is a normal symmetric surface with
...
cohomology yields an unshifted summand $\IC_X\subset\Alt^2(\IC_X)$ in the
convolution category, while a generic unitary twist has Hodge multiplicities
$(2,3,2)$.  These two inputs determine the Tannaka pair: the connected group
is $G_2$ on its standard seven-dimensional representation, and the same
exterior-square morphism rules out the only possible disconnected extension.
Thus the full convolution Tannaka group is $G_2$, realizing the $G_2$ case of
Litt Problem No.~13.
\end{abstract}
...
\cite{KraemerWeissauer2015}.  Litt Problem No.~13 asks whether the four
exceptional simple types
\[
  G_2,\qquad E_7,\qquad E_8,\qquad F_4
\]
can occur in this manner \cite{Litt13}.  We treat the $G_2$ case and make no
claim about the other three.  The construction necessarily uses a singular
theta surface; this point matters because the exclusion results for smooth
subvarieties in \cite{KLM2026} do not apply here.
...
The point where most of the geometry enters is the generic pair-incidence
fibre.  For a geometric generic point $x$ of
...
For ease of reference, the proof has the following dependency chain.
...
We next locate the remaining critical points of the norm map by separating
...
with exceptional elliptic curve $E_0$ and $E_0^2=-3$.  We now verify the
local behaviour of the pencil $\Gamma=|\kappa|$ at the vertex.  Projection
formula gives
\[
...
\]
We spell out the basis argument because it will be used in the norm
calculation.  On $D$ we have $x=Y=w^3$ and
...
\end{equation}
This is the coefficientwise form of the square-class identity.
