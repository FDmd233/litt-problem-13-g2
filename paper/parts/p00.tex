...
\begin{abstract}
Starting from the cyclic triple covers
\[
  C:\quad w^3=y-c,\qquad c\ne0,
\]
of elliptic curves $y^2=x^3+Ax+B$, we construct a family of complex abelian
threefolds whose Prym polarization has type $(1,1,3)$.  The restricted
centred theta divisor $X$ is a normal symmetric surface with a unique simple
elliptic singularity of type $\wt E_7$, and $\chi(\IC_X)=7$.  For general
parameters the pair-incidence curve $X\cap(-x-X)$ has exactly two
nonexceptional components.  Their top cohomology produces an unshifted
summand $\IC_X\subset\Alt^2(\IC_X)$ in the convolution category.  On the
other hand, a generic unitary twist has Hodge multiplicities $(2,3,2)$.
Together these constraints leave only the standard seven-dimensional
representation of $G_2$, and the argument also excludes an extra disconnected
component of the Tannaka group.  Thus the full convolution Tannaka group is
$G_2$, giving the $G_2$ case of Litt Problem No.~13.
\end{abstract}
...
claim about the other three.  One feature of the construction is worth
stressing at the outset: the theta surface is singular.  Consequently the
exclusion results for smooth subvarieties in \cite{KLM2026} do not apply to
this example.
...
\item $\chi(\IC_{X_b})=7$, and there is an isomorphism of a reductive
      group with its faithful representation
\[
  \bigl(G(\IC_{X_b}),\omega(\IC_{X_b})\bigr)\simeq(G_2,V_7),
\]
      where $V_7$ is the standard seven-dimensional representation of
      $G_2$.  In particular, the full convolution Tannaka group is connected and equal to
      $G_2$.
\end{enumerate}
