...
We next locate the remaining critical points of the norm map.  This is a
local calculation on the symmetric cube, and it is useful to separate the
possible multiplicity patterns.  A divisor containing an unramified point
...
with exceptional elliptic curve $E_0$ and $E_0^2=-3$.  The pencil
$\Gamma=|\kappa|$ has multiplicity one at the vertex.  If $t$ is a local
parameter on the pencil at $D_0$, then each ramification coordinate has an
expansion
\[
 z_i=\lambda_i t+O(t^2),
\]
with not all $\lambda_i$ zero.  Because $\Gamma\subset N$, the tangent
direction $[\lambda_1:\lambda_2:\lambda_3]$ lies on the exceptional cubic
$\lambda_1^3+\lambda_2^3+\lambda_3^3=0$.  This is the unique tangent
direction of $\Gamma$ at the vertex; the multiplicity-one statement then
shows that its strict transform meets $E_0$ in one reduced point,
transversely.
