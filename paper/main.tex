...
\]
The second assertion is taken in the convolution Tannakian category.  We prove
it by identifying two components of a generic pair-incidence curve, spreading
those components over a relative open set, and extracting the divisor-supported
summand of pair convolution.  The component calculation rests on an exact
norm--conic identity over a rational function field.  Finally, the Hodge
cocharacter has weight multiplicities $(2,3,2)$, which, together with the
exterior-square direct summand, identifies the full group with $G_2$ on its
standard seven-dimensional module.
\end{abstract}
...
The Singular/Rabin wrapper listed below evaluates the specialization at
$(a,b,c)=(1,2,3)$ and establishes geometric integrality of the residual-octic
fibre used in
\cref{lem:residual-octic-integral}.  Exact finite-field checks in
