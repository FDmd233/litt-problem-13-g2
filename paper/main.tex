...
\begin{abstract}
A family of complex abelian threefolds is constructed from the cyclic covers
\[
  C:\quad w^3=y-c,\qquad c\ne0,
\]
of elliptic curves $y^2=x^3+Ax+B$.  The associated Prym variety carries a
polarization of type $(1,1,3)$, and the restricted centred theta divisor
$X$ is a normal symmetric surface with one simple elliptic singularity of
type $\wt E_7$.  Its intersection complex has Euler characteristic seven.
For general parameters, the generic pair-incidence curve
\[
  X\cap(-x-X)
\]
has exactly two nonexceptional components.  This yields an unshifted
summand $\IC_X\mid\Alt^2(\IC_X)$ in the convolution category.  A generic
unitary twist has Hodge multiplicities $(2,3,2)$.  These two facts rule out
the other irreducible orthogonal groups in dimension seven and identify the
full convolution Tannaka group with $G_2$ on its standard representation.
The result gives the $G_2$ case of Litt Problem No.~13.
\end{abstract}
...
No claim is made here for the other three types.  The singularity of the
theta surface is essential: the exclusion results for smooth subvarieties in
\cite{KLM2026} do not apply to the surface constructed below.
...
The present version incorporates two corrections.  The construction is made
with $c\ne0$, since the specialization $c=0$ has an additional involution.
It also corrects the first-cohomology calculation at the simple-elliptic
point: the minimal resolution has $p_g(S)=5$ and $q(S)=4$.  The proof of the
generic-twist Hodge vector is correspondingly rewritten and no longer uses
$H^1(X,\QQ)=IH^1(X,\QQ)$.  The earlier isotrivial-CM assertion for the Prym
family is not used.
...
\]
A separate theta-group argument gives
\[
 p_g(S)=5,\qquad q(S)=4.
\]
...
\paragraph{Explicit algebra.}
For clarity, the polynomial identities used in the pair-incidence argument
are isolated in Appendix~\ref{app:exact-norm}.  The determinant
factorization, the square-class identity, and the characteristic-$13$
witnesses are stated there in a form that can be checked directly.  The
repository contains a short exact-arithmetic script that repeats the same
calculations; no result depends on running it.
...
\]
has order nine.  We first show that every point of $K(L)$ lies on $X$.  For
...
We next check that all five canonical sections vanish at the singular point.
The two-dimensional subspace coming from $H^0(P,L)/\CC s$ does so by
\eqref{eq:basepoint-origin}.  For the remaining three directions, let $v$ be
a translation-invariant vector field on $P$.  Differentiating a local
equation for $s$ in the direction $v$ and restricting to $X$ gives a section
...
\]
Indeed, if $g_{ij}$ are transition functions for $L$, the corresponding
\v{C}ech cocycle is $v(g_{ij})/g_{ij}$.  This is the differential at the
origin of the polarization morphism
...
Write $E=E_{\exc}$.  Since $K_X$ is Cartier, one may write
$K_S=\rho^*K_X+aE$.  Adjunction on the elliptic curve $E$ gives
$(K_S+E)\cdot E=0$, while $\rho^*K_X\cdot E=0$ and $E^2=-2$; hence
$a=-1$ and
\begin{equation}
 K_S=\rho^*K_X-E.
\label{eq:canonical-resolution}
\end{equation}
For a simple elliptic singularity of type $\wt E_7$, the maximal-ideal
cycle on the minimal resolution is the reduced exceptional curve $E$; hence
\[
 \rho_*\mathcal O_S(-E)=\mathfrak m_0.
\]
For completeness, this can be read directly from the standard local form
$u^2+v^4+w^4+\lambda v^2w^2=0$ of a $\wt E_7$ singularity
\cite{Laufer1977,Pinkham1977}: its minimal resolution is the weighted blowup
with weights $(2,1,1)$, and positive $E$-valuation is exactly vanishing at
the origin.  By the projection formula,
\[
...
\]
The remaining Betti and Hodge numbers follow from $e(S)=8$.
\end{proof}

\begin{remark}[Cohomological check]
\label{rem:q-correction}
Artin vanishing for $P\setminus X$ gives
$H^1(P,\QQ)\simeq H^1(X,\QQ)$.  The identification
$H^1(X,\QQ)\simeq IH^1(X,\QQ)$ asserted in an earlier revision is false.  Although the kernel and cokernel of
$\QQ_X[2]\to\IC_X$ are supported at $0$, their degree-zero
hypercohomology enters the connecting map in the long exact sequence.  In
fact \eqref{eq:semismall} and \cref{prop:resolution-irregularity} give
\[
 \dim H^1(X,\QQ)=6,
 \qquad
 \dim IH^1(X,\QQ)=\dim H^1(S,\QQ)=8.
\]

There is also a useful equivariant check.  After removing the proper closed
locus on which a branch point is $3$-torsion, the deck transformation
$\tau$ has eight smooth fixed points on $X$ besides the origin.  Indeed, a
smooth fixed point has a unique effective divisor, and a $\tau$-invariant
divisor of degree three is either a free orbit $\pi^{-1}(q)$, with
$q\in E[3]$, or is supported on ramification points.  The free orbits give
nine fixed points, one of which is the origin; the remaining ramification
possibilities are excluded by the distinctness of the three branch points
and the non-$3$-torsion assumption.  In the local model
$z_1^3+z_2^3+z_3^3=0$, $\tau$ acts by simultaneous scalar multiplication,
so the exceptional elliptic curve is fixed pointwise.  Thus the fixed locus
on $S$ has Euler characteristic eight.  If one incorrectly imposed
$q(S)=3$, then $b_2(S)=18$, the trace of $\tau$ on $H^1(S)$ would be $-3$,
and the injection
$H^2(P,\QQ)\hookrightarrow H^2(S,\QQ)$ from hard Lefschetz would force the
Lefschetz number to be $14$ or $17$, not $8$.  This is an independent way
to detect the erroneous $H^1=IH^1$ step.  With the corrected value $q(S)=4$,
one has $b_2(S)=22$.  The extra two dimensions in $H^1(S)$ come from the
exceptional elliptic curve, on which $\tau$ acts trivially; hence
$\operatorname{tr}(\tau\mid H^1(S))=-1$.  Compatibility with the fixed-locus
formula is then exact: the trace on $H^2(S)$ is $4$, and
\[
  L(\tau,S)=1-(-1)+4-(-1)+1=8=\chi(S^\tau).
\]
\end{remark}
...
holomorphic line bundle $\alpha\in\operatorname{Pic}^0(P)$.  We use the
same symbol $\alpha$ for the latter.
...
$\rho_*\mathcal O_S(-E)=\mathfrak m_0$ yields
\begin{equation}
...
subset of $\operatorname{Pic}^0(P)$.  To see nonemptiness, use the
...
$\operatorname{Pic}^0(P)$ is a real-analytic isomorphism.  Hence the generic
vanishing locus and $W$ meet in a nonempty Euclidean open subset $\Omega$ of
the unitary character torus.
...
\]
For a unitary local system, the twisted Hodge decomposition in degree two is
\[
...
It remains only to see that the factor occurs once.  Normalize $e_1=1$ by
\cref{lem:dominant-chart}.  At
\[
 (c,\lambda,e_1,e_2,e_3)=(1,1,1,0,2)
\]
modulo $13$, direct substitution in \eqref{eq:cyclic-norm} gives
\[
 4F_8(1,1,1)=12\ne0\quad\text{in }\mathbf F_{13}.
\]
Thus $u-v\nmid F_8$.  The same scaling transports the identity back to
$K_0$.
\end{proof}
...
\begin{proof}
The explicit formulas in Appendix~\ref{app:exact-norm} reconstruct $F$,
$\Delta$, and the coefficients of $A_{\mathrm c}$ and $B_{\mathrm c}$ from
the shifted curve.  Substitution gives \eqref{eq:norm-square-identity} in
...
Put $\mu(z)=h(z,z^{-1})$.  The following short argument places the antidiagonal cocharacter in the
common derived group; spelling it out avoids any ambiguity between the Hodge
and perverse Tannaka groups.
...
\section{Explicit norm identities and finite-field witnesses}\label{app:exact-norm}

This appendix records the algebra used in \cref{sec:pair-splitting}.  The
identities below are part of the proof: they can be checked by direct
substitution in the indicated polynomial rings.  The optional repository script
\path{code/g2_shifted_norm_certificate.py} repeats the same substitutions with
exact arithmetic and serves only as an independent check.
...
$L_c=4\lambda^2\ell$.  The required coefficients are as follows:
\[
 \begin{aligned}
...
\[
 F=A_{\mathrm c}^2+3\Delta B_{\mathrm c}^2.
\]
This is the identity used in \cref{prop:ordering-cover}.  Direct expansion of the same normalized formulas gives
\[
 \det(M_s)=(u-v)F_8,
 \qquad
 \deg F_8=8,
\]
with $F_8$ homogeneous.  The structural factor has multiplicity one: at the
specialization $(c,\lambda,e_1,e_2,e_3)=(1,1,1,0,2)$ modulo $13$ one finds
\[
 4F_8(1,1,1)=12\ne0.
\]
...
For \cref{lem:residual-octic-integral}, reduce
\[
 (c,\lambda,e_1,e_2,e_3)=(1,1,1,0,2)
\]
modulo $13$.  The elliptic and branch discriminants are $8$ and $4$.  Put
\[
...
\[
 v^{13^8}-v\equiv0.
\]
The first remainder is coprime to $h$; for example, in
$\mathbf F_{13}[v]$ the explicit B\'ezout identity
\[
 a(v)h(v)+b(v)r(v)=1
\]
holds with
...
\end{aligned}
\]
Rabin's criterion therefore proves that $h$ is irreducible over
$\mathbf F_{13}$.  The same specialization has the smooth point
$(u,v)=(1,6)$ with gradient $(2,0)$.  For
...
For reproducibility, the optional verification script can be run with
\begin{verbatim}
