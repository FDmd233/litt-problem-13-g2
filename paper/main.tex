...
\begin{abstract}
We exhibit a nonempty family of complex abelian threefolds $P$ with integral
...
\]
The second assertion is in the convolution Tannakian category.  We prove it by
...
For readers checking the argument, the proof has five interfaces, each with a
separate output used by the next one.
...
\end{enumerate}
The exact scripts are used only at the three explicitly marked algebraic
interfaces in (3): determinant derivation, the ordering-cover identity, and
...
construction we first extend the underlying perverse sheaves from $\QQ$ to
...
$\CC$.
We use $\PP(\mathcal E)=\operatorname{Proj}(\operatorname{Sym}
\mathcal E^\vee)$, so it parametrizes lines in the fibres of $\mathcal E$.
For an integral scheme $Y$, we write $k(Y)$ for its function field; for a
point $y$, we write $k(y)$ for its residue field.

\section{Tannakian conventions}

Let $A$ be a complex abelian variety and let $m:A\times A\to A$ be addition.
For constructible complexes with coefficients in $\QQ$ or $\CC$ we write
\[
K*L=Rm_*(K\bo L).
\]
Set
...
We denote by $\delta_0$ the skyscraper perverse sheaf at the origin; its
...
$\chi(\IC_Z)>0$, we denote by $G(\IC_Z)$ the Tannaka group of the category
...
\]
The action on the tautological module is faithful by construction.  We
suppress the scalar extension from notation inside Tannakian expressions.
We write $\IC_Z^H$ for the polarizable pure Hodge module whose underlying
...
\end{equation}
This identifies our convention with the Prym used in the polarization formula
below.
...
a proper morphism $\rho:S\to X$; the curve $E_{\exc}$ maps to the origin.  We
...
\begin{proof}
We compute $e(N)$ from the finite map $N\to|\eta^3|\simeq\PP^2$.
...
We return to the family over $\cB$.  Let $\cE\to\cB$ be the Weierstrass
...
We henceforth drop the primes when referring to the normalized chart.
\end{proof}
...
Thus, with respect to the coordinates $(1,x,y)$, our convention is
\[
 M_s=
 \begin{pmatrix}
...
pulled back to $T'$, and we suppress this base change from the notation.
The line $\ell_i$ then
...
We now justify the passage from this point to characteristic zero.  Enlarge
...
Here we use $\CC(T)=\CC(\cM)$ from
\eqref{eq:chart-function-field} to identify the two geometric generic
fibres.
\end{proof}
...
define two sections of $q$.  We write $\cI^{\mathrm{ne}}$ for their
complement.
...
\begin{proof}
During the proof we repeatedly restrict all schemes to a nonempty open of
...
any two fibre functors over $\CC$ are noncanonically isomorphic, we identify
...
The author thanks Daniel Litt for encouraging a more checkable exposition.
OpenAI language-model tools assisted with drafting, symbolic-code review, and
proof auditing.  The computer-assisted assertions used in the proof are exact,
are delimited in the Introduction and Appendix, and can be independently
reproduced from the accompanying source files.
