...
direct summand $\IC_X$ of $\Alt^2(\IC_X)$ in the convolution category.  A generic
...
decomposition theorem show that $\IC_X$ is a direct summand of
$\Alt^2(\IC_X)$ in the convolution Tannakian category.  If
...
\end{equation}
The ideal that appears after resolution can be identified directly.  Since
...
It remains to show that this factor occurs with multiplicity one.  Normalize $e_1=1$ by
\cref{lem:dominant-chart}.  At
\[
 (c,\lambda,e_1,e_2,e_3)=(1,1,1,0,2)
\]
modulo $13$, Appendix~A.1 evaluates all six conic coefficients on the
one-parameter slice $(u,v,z)=(1+t,1,1)$ and obtains
\eqref{eq:appendix-simple-structural-factor}.  At $t=0$ its residual factor
...
\[
 ...
 \end{aligned}
\]
Appendix~A.3 displays all Frobenius remainders $v^{13^j}\bmod h$ for
$0\le j\le8$ and an explicit B\'ezout identity proving that
$h$ is coprime to $v^{13^4}-v$.  The elementary factor-degree criterion
proved there therefore makes $h$ irreducible over $\mathbf F_{13}$.
...
For reference, the degree bookkeeping is
\[
\begin{array}{c|c|c}
...
\section{Exact norm identities and finite-field witnesses}\label{app:exact-norm}

This appendix records the algebra used in \cref{sec:pair-splitting}.
Rather than expand the residual octic over the full parameter space, we give
the coefficient formulas and the finite identities at the points where they
enter the proof.  These formulas determine each calculation uniquely and make
the required coefficient checks local and finite.
...
\end{align*}
These six coefficients determine $M_s$ and hence $F_8$ without any omitted
choice.  In particular, for
\[
 M_s=\begin{pmatrix}
 q_{00}&q_{01}/2&q_{02}/2\\
 q_{01}/2&q_{11}&q_{12}/2\\
...
\[
 \Delta=-4\left(q_{00}q_{11}-(q_{01}/2)^2\right).
\]
With no further definitions, substitution in
\eqref{eq:appendix-determinant-expansion} and collection in powers of $z$
gives the single polynomial identity
\begin{equation}
 16\lambda^2L_c\,\widetilde F(z)=R(z)^2+3\Delta N(z)^2.
\label{eq:appendix-primitive-square-identity}
\end{equation}
This is an identity in
$\mathbf Z[c,\lambda,e_2,e_3,u,\lambda^{-1}][z]$.  All coefficients on both
sides are given in A.1--A.2, so \eqref{eq:appendix-primitive-square-identity}
reduces to a coefficient-by-coefficient comparison in that ring.
...
\]
We verify irreducibility of $h$ from the displayed Frobenius data.  In
$\mathbf F_{13}[v]/(h)$ one may reduce powers using only
\begin{equation}
 v^8=3v^7+2v^6-3v^5-4v^4-4v^3+3v^2+5v+2.
\label{eq:appendix-h-reduction}
\end{equation}
Put $r_j\equiv v^{13^j}\pmod h$, with representatives of degree at most
seven.  Frobenius, followed by repeated use of
\eqref{eq:appendix-h-reduction}, gives the complete table
\begin{align*}
r_0={}&v,\\
...
r_8={}&v.
\end{align*}
Thus $h$ divides $v^{13^8}-v$.  Moreover
$r_4-v=r(v)$, where
\[
...
\subsection*{A.4. The admissibility witness}
...
condition~(iii) of \cref{def:admissible-locus}.  Finally, the canonical-quadric
argument of \cref{prop:theta-resolution} is valid in characteristic $13$ and
shows that $|\kappa|$ is the unique $g^1_3$.  A norm line from this pencil has
zero $x$-coefficient, whereas $L_0,L_1,L_2$ all have nonzero $x$-coefficient.
Thus their inverse degree-three divisors are not linearly equivalent to
$\kappa$, hence lie in the $h^0=1$ Abel locus and map to the smooth theta
locus.  This verifies condition~(iv) and completes the finite-field witness
used in \cref{lem:admissible-nonempty}.
