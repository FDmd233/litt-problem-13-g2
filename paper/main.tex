...
\cite{Macdonald1962} gives $K_{C^{(3)}}=a_C^*\Theta_C$.  The norm fibre has
trivial normal bundle, so adjunction and restriction to the Prym translate
give $K_N\equiv a^*L$.  Since $a$ contracts $\Gamma$, one has
...
\[
 E_{\exc}^2=-2.
\]
The composite $T\to N\to X$ is constant on $\wt\Gamma$, so it factors through
...
\[
 7=\deg\Lambda_X+\deg\Lambda_0=\deg\Lambda_X+1,
\]
and hence
\begin{equation}
...
Choose a unitary rank-one local system $\mathcal L_\alpha$ on $P$ whose
associated line bundle $\alpha\in\operatorname{Pic}^0(P)$ lies in the
intersection of the coherent generic-vanishing opens for $\alpha$ and
$\alpha^{-1}$ and the perverse generic-vanishing open below.  This
intersection is nonempty.  Coherent generic vanishing
...
point $x\in X$.  In the original short-Weierstrass coordinates, its norm is
a general line $y=mx+n$, with $m\ne0$.  Put
\[
...
\]
Conversely, a general pair whose norm line is $y=mx+n$ recovers these
parameters by the first coordinate change.  Thus
$(m,e_1,e_2,e_3)$ is a dominant four-parameter chart of the actual pair
space.  The calculation below is made before adjoining $m$; the cubic
extension has odd degree and therefore cannot split a nontrivial quadratic
factor-ordering cover.

There is a scaling redundancy in the primed coordinates
\[
...
\[
 ...
 \qquad (a,b,c)=(1,2,3),
\]
dehomogenize by $z=1$ and denote the resulting polynomial by $H(u,v)$.
Modulo $13$, it has $v$-degree eight with constant leading coefficient
$-5$, and its monic slice at $u=1$ is
\[
...
If $H=H_1H_2$ with both factors nonconstant, their leading
$v$-coefficients in $\mathbf F_{13}[u]$ would have product $-5$.  Both are
therefore units.  A factor of $v$-degree zero would then itself be a unit,
while every factor of positive $v$-degree retains that degree after $u=1$.
Thus a nontrivial factorization of $H$ would specialize to a nontrivial
factorization of $h$, a contradiction.  Hence $H$ is irreducible.  Its total
degree remains eight after dehomogenization, so the homogeneous octic is not
divisible by the homogenizing variable.  Any nontrivial homogeneous
factorization would therefore dehomogenize to a nontrivial factorization of
$H$.  Hence the homogeneous octic is irreducible as well.

The same certificate gives the rational point $(u,v)=(0,2)$ with gradient
$(-2,0)$.  If this irreducible finite-field curve were geometrically
reducible, Frobenius would permute its geometric components transitively.
A rational point is Frobenius-fixed, so it would lie on every component and
would be singular.  The displayed smooth point excludes this, proving that
the projective octic is geometrically integral.  Openness of geometric
integrality in the flat projective family constructed in
\cref{sec:open-family} proves the generic assertion.
...
modulo $13$, an exact witness in the displayed universal basis
...
connected curve $X\cap(t-X)$.  Indeed, the difference map
$X\times X\to P$ is surjective because two translates of the ample divisor
$X$ always meet.  Generic smoothness on $X_{\reg}\times X_{\reg}$, together
with avoidance of the two loci involving the isolated singular point, gives
smoothness of the general fibre.  Since $t\notin X$ and
...
coset, take $g\in G\cap(-G_2)$ and write $g=(-I)h$ with
$h\in G_2=G^0\subset G$.  Then $-I=gh^{-1}\in G$.  But $-I$ acts by $-1$ on
$V$ and by $+1$ on
...
universal effective divisor over $\cM$.  Use the integral four-parameter
chart $(m,e_1,e_2,e_3)$ constructed in \cref{sec:pair-splitting}; equivalently,
it is the cubic cover $m^3\lambda=1$ of the unnormalized
$(\lambda,e_1,e_2,e_3)$ space.  The explicit inverse obtained from the norm
line $y=mx+n$ proves that this chart maps dominantly to $\cM$.  After
restricting its domain, let $T\to\cM$ be this integral dominant chart.  The
slice $e_1=1$ is used only for the function-field calculation and does not
lower the dimension of $T$.  After the local-freeness shrinking, write
$p:\cC_T\to T$ for the relative curve and $D$ for its universal degree-three
divisor.  Then
...
\]
is projective and flat over $T_{R_0}$.  Indeed, after deleting the closed
locus where its defining octic is identically zero on a projective-space
fibre, $F_8$ is a fibrewise non-zero-divisor.  The sequence
\[
 0\longrightarrow\mathcal O(-8)\xrightarrow{F_8}\mathcal O
 \longrightarrow\mathcal O_{\cR_{R_0}}\longrightarrow0
\]
remains exact after base change.  The local criterion for flatness therefore
shows that $\cR_{R_0}\to T_{R_0}$ is flat and of finite presentation.  The
first modulo-$13$ certificate in
\cref{prop:universal-identity} is a geometrically integral fibre of this
family.  Geometric integrality is open for a flat projective family
\cite[IV$_3$, Th\'eor\`eme~12.2.4\textup{(viii)}]{EGA4}; the
...
closures $\cA_1,\cA_2\subset\cI$.  Factor transposition exchanges these
closures because it exchanges their generic fibres.  The pair-incidence
morphism is projective, hence proper and of finite presentation.  Generic flatness
(Stacks Project, Tag~\href{https://stacks.math.columbia.edu/tag/052A}{052A})
and generic spreading of geometric reducedness and irreducibility
(Tags~\href{https://stacks.math.columbia.edu/tag/0578}{0578} and
\href{https://stacks.math.columbia.edu/tag/0559}{0559}) show that, after
replacing $\cM$ by a nonempty open $\cM^\circ$, the two closures are flat with
geometrically integral curve fibres.  The boundary-exhaustion argument also
shows that the entire geometric generic fibre of $\cI\to\cM$ is reduced.
Applying Tag~\href{https://stacks.math.columbia.edu/tag/0578}{0578} once more
and shrinking $\cM^\circ$ makes every geometric fibre of $\cI$ reduced.

Because $\cI\to\cM^\circ$ is projective and the $\cA_i$ are flat, their closed
immersions define two sections of the relative Hilbert scheme
$\operatorname{Hilb}_{\cI/\cM^\circ}$.  This Hilbert scheme is separated over
$\cM^\circ$, so the equalizer of the two sections is closed.  It omits the
generic point because the two generic components are distinct.  Delete this
equalizer; then $(\cA_1)_x$ and $(\cA_2)_x$ are distinct for every geometric
$x\in\cM^\circ$.

Let $\cI^{\mathrm{ne}}$ be the complement in $\cI$ of the two exceptional
sections $y=0$ and $y=-x$.
Give $\cA=\cA_1\cup\cA_2$ its scheme-theoretic union structure, and let
$\mathcal J$ be the ideal sheaf of
$\cA\cap\cI^{\mathrm{ne}}$ in $\cI^{\mathrm{ne}}$.  The geometric generic
fibre of $\cI^{\mathrm{ne}}$ is reduced and is exactly the union of the two
generic components, so $\mathcal J$ vanishes there.  By Chevalley's theorem,
the image of $\operatorname{Supp}(\mathcal J)$ in the noetherian integral
scheme $\cM^\circ$ is constructible and omits the generic point; its closure is
therefore a proper closed subset.  Removing this closure makes
$\mathcal J=0$.  Thus, after this final shrinking,
\[
 \cI^{\mathrm{ne}}=\cA\cap\cI^{\mathrm{ne}}
\]
scheme-theoretically.  In particular, there is no additional, embedded, or
vertical nonexceptional component in any fibre over $\cM^\circ$.  Since those
fibres are reduced and the two displayed integral curve fibres are distinct,
they are exactly the two nonexceptional irreducible components in every
geometric fibre.

The morphism $\cM^\circ\to\cB$ is the restriction of a smooth morphism and is
therefore open (Stacks Project,
Tag~\href{https://stacks.math.columbia.edu/tag/056G}{056G}).  Its image
\[
...
check.  Starting again from the cyclic norm, the provenance verifier also
reports
\[
 \begin{gathered}
...
\]
The second sample uses a kernel basis $(K_0,K_1,K_2)$ related to the universal
basis by
\[
 K_0=s_0,\qquad K_1=5s_0+4s_1-s_2,\qquad K_2=s_0+s_1.
\]
It checks exactly that the internal point $(1:0:9)$ is the universal point
$(10:9:0)$ and that the corresponding conic has coefficient vector
$(4,3,1,2,3,12)$ modulo $13$.  The genuine-open certificate then reports the
...
\end{aligned}
\]
in $\mathbf F_{13}$.  The source equations, coefficient formulas, and
...
\end{itemize}
Their generated inputs are in \path{code/generated/}.  The provenance verifier derives the saved Singular
