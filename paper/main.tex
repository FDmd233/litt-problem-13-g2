...
Prym polarization has type $(1,1,3)$.  Each theta section $X$ is normal and
has one simple elliptic singularity of type $\wt E_7$; its intersection
complex $K=\IC_X$ satisfies
\[
  \chi(K)=7,
  \qquad
...
resulting divisor-supported summand.  Their calculation rests on an exact
norm--conic identity over a rational function field.  Finally, the Hodge
cocharacter has weight multiplicities $(2,3,2)$; together with the
exterior-square summand, this identifies the full group with $G_2$ on its
standard seven-dimensional module.  Thus Litt Problem No.~13 has an
affirmative answer for $G_2$.
\end{abstract}
...
the full convolution Tannaka group.
\end{theorem}
...
splits the double cover that orders the two line factors.  A relative Abel
inverse identifies those ordered factors with two components of $C_x$, and a
boundary argument shows that there are no further nonexceptional components.
...
All varieties are defined over $\CC$.  Intersection complexes, cohomology, and
Hodge modules retain their canonical $\QQ$-structures.  For the Tannakian
...
$M\mapsto M^{-1}$.  Pulling back to $P$ therefore gives, as closed subschemes,
\begin{equation}
 X=-X.
\label{eq:symmetry}
\end{equation}
...
$a(D)=M$.  The finite morphism $\pi$ induces a finite morphism
...
quadric.  Consequently $\kappa$ is the unique $g^1_3$ on $C$.  Riemann--Kempf
 ...
 $\operatorname{mult}_0(\Theta_\kappa)=h^0(C,\kappa)=2$.  Thus a local
 equation of $\Theta_\kappa$ at the origin has no linear term.  Its restriction
 to the smooth threefold $P$ is nonzero by the paragraph following the
 definition of $a:N\to X$, and it still has no linear term.  The restricted
 divisor $X$ is therefore singular at the origin.  Consequently
\begin{equation}
 \Sing(X)=\{0\}.
...
the corresponding base changes.  The divisor $\cX$ is
...
\begin{proposition}[Triviality of the ordering cover]
...
\]
there are $A,B\in K_1[z]$ with $\deg A=3$, $\deg B\le2$, and
\begin{equation}
 F=A^2+3\Delta B^2.
\label{eq:norm-square-identity}
\end{equation}
After extending scalars from $\QQ$ to $\CC$, the finite \emph{\'etale}
double cover that orders the two distinct line factors of the generic
rank-two conic on $\mathscr R$ is split.
\end{proposition}
...
\[
 F=(A+tB)(A-tB).
\]
The formula for $B$ in \cref{app:certificate} has nonzero $z^2$-coefficient.
Since
$\deg_z(B)\le2<6=\deg_z(F)$ and $F$ is irreducible by
\cref{lem:normalized-function-field}, $B$ remains nonzero in the function
field of $F=0$.  Likewise, $A$ has degree three and is nonzero there.  Thus
$A/B\ne0$, while \eqref{eq:norm-square-identity} gives
\[
 -3\Delta=(A/B)^2\ne0.
\]
The ordering algebra is therefore \'etale at the generic point, and its two
rational roots $t=\pm A/B$ are distinct.
...
$\Delta$ defines the intrinsic ordering cover.  The constant $-3$ is a
square over $\CC$, and \cref{lem:normalized-function-field} carries this
splitting to the full generic residual curve and then to $T$.
\end{proof}
...
\'etale factor-ordering cover has two rational sections at the generic point
of $\mathscr G$, given on the principal-minor chart by $t=\pm A/B$.
...
\cref{thm:uniform-pair-splitting}, and fix $b\in\cB^\circ(\CC)$.  The
...
\]
The leading coefficients agree because $F$ and $A$ are monic.  The identity
$F=A^2+kB^2$ is therefore equivalent to the remaining six coefficient
equalities:
\[
\begin{aligned}
c_5&=2a_2,\\
...
$4\lambda^2L_0^2$ from $F-A^2-kB^2$ and obtains the zero polynomial.  These
